...
from engine.emr import prepare_step_paths, pyspark_run_script_contents_step
from dart.util.s3 import s3_copy_recursive


def run_pyspark_script(emr_engine, datastore, action):
    """
    ...
    steps = prepare_run_pyspark_script_steps(dry_run, datastore, action, action.data.args['script_contents'])
...
def prepare_run_pyspark_script_steps(dry_run, datastore, action, script_contents):
    ...
    try:
        ...
        steps = [pyspark_run_script_contents_step(script_contents, s3_step_path, local_step_path, action.id, 1, 1)]
