    ...
    WITH RECURSIVE entity_graph(type, id, name, state, sub_type, related_type, related_id, related_is_a, dart_depth) AS (
        ...
        SELECT t.*
          ...
          JOIN LATERAL (
                    ...
                    -- ===============================================
                    SELECT 'action', e.id, e.data ->> 'name', e.data ->> 'state', e.data ->> 'action_type_name', 'dataset', g.id, 'PARENT', LEAST(2,(g.dart_depth+1))
                      FROM action e
                    ...
                    SELECT 'subscription', e.id, e.data ->> 'name', e.data ->> 'state', NULL, 'dataset', g.id, 'PARENT', LEAST(2,(g.dart_depth+1))
                      FROM subscription e
                     WHERE g.type = 'dataset'
                    ...
                    -- ===============================================
                    SELECT 'dataset', e.id, e.data ->> 'name', NULL, NULL, 'action', g.id, 'CHILD', LEAST(2,(g.dart_depth+1))
                      FROM dataset e
                      JOIN action a
                        ON a.id = g.id
                    ...
                    SELECT 'subscription', e.id, e.data ->> 'name', e.data ->> 'state', NULL, 'action', g.id, 'CHILD', LEAST(2,(g.dart_depth+1))
                      FROM subscription e
                      JOIN action a
                        ON a.id = g.id
                       AND a.data ->> 'state' = 'TEMPLATE'
                    ...
                    (SELECT 'workflow', e.id, e.data ->> 'name', e.data ->> 'state', NULL, 'action', g.id, 'CHILD', LEAST(2,(g.dart_depth+1))
                      FROM workflow e
                      JOIN action a
                        ON a.id = g.id
                       AND a.data ->> 'state' = 'TEMPLATE'
                  ...
                  ORDER BY CAST(a.data ->> 'order_idx' AS FLOAT))
                    ...
                    -- ===============================================
                    (SELECT 'action', e.id, e.data ->> 'name', e.data ->> 'state', e.data ->> 'action_type_name', 'workflow', g.id, 'PARENT', LEAST(2,(g.dart_depth+1))
                      FROM action e
                  ...
                  ORDER BY CAST(e.data ->> 'order_idx' AS FLOAT))
                    ...
                    SELECT 'datastore', e.id, e.data ->> 'name', e.data ->> 'state', e.data ->> 'engine_name', 'workflow', g.id, 'CHILD', LEAST(2,(g.dart_depth+1))
                      FROM datastore e
                      JOIN workflow w
                        ON w.id = g.id
                    ...
                    SELECT 'trigger', e.id, e.data ->> 'name', e.data ->> 'state', e.data ->> 'trigger_type_name', 'workflow', g.id, 'CHILD', LEAST(2,(g.dart_depth+1))
                      FROM trigger e
                     WHERE g.type = 'workflow'
                    ...
                    SELECT 'trigger', e.id, e.data ->> 'name', e.data ->> 'state', e.data ->> 'trigger_type_name', 'workflow', g.id, 'PARENT', LEAST(2,(g.dart_depth+1))
                      FROM trigger e
                     WHERE g.type = 'workflow'
                    ...
                    -- ===============================================
                    SELECT 'workflow', e.id, e.data ->> 'name', e.data ->> 'state', NULL, 'trigger', g.id, 'PARENT', LEAST(2,(g.dart_depth+1))
                      FROM workflow e
                      JOIN trigger t
                        ON t.id = g.id
                    ...
                    SELECT 'workflow', e.id, e.data ->> 'name', e.data ->> 'state', NULL, 'trigger', g.id, 'CHILD', LEAST(2,(g.dart_depth+1))
                      FROM workflow e
                      JOIN trigger t
                        ON t.id = g.id
                    ...
                    SELECT 'subscription', e.id, e.data ->> 'name', e.data ->> 'state', NULL, 'trigger', g.id, 'CHILD', LEAST(2,(g.dart_depth+1))
                      FROM subscription e
                      JOIN trigger t
                        ON t.id = g.id
                    ...
                    SELECT 'event', e.id, e.data ->> 'name', e.data ->> 'state', NULL, 'trigger', g.id, 'CHILD', LEAST(2,(g.dart_depth+1))
                      FROM event e
                      JOIN trigger t
                        ON t.id = g.id
                    ...
                    SELECT 'trigger', e.id, e.data ->> 'name', e.data ->> 'state', e.data ->> 'trigger_type_name', 'trigger', g.id, 'CHILD', LEAST(2,(g.dart_depth+1))
                      FROM trigger e
                      JOIN trigger t
                        ON t.id = g.id
                     WHERE g.type = 'trigger'
                    ...
                    SELECT 'trigger', e.id, e.data ->> 'name', e.data ->> 'state', e.data ->> 'trigger_type_name', 'trigger', g.id, 'PARENT', LEAST(2,(g.dart_depth+1))
                      FROM trigger e
                      JOIN trigger t
                        ON t.id = g.id
                     WHERE g.type = 'trigger'
                    ...
                    -- ===============================================
                    SELECT 'dataset', e.id, e.data ->> 'name', e.data ->> 'state', NULL, 'subscription', g.id, 'CHILD', LEAST(2,(g.dart_depth+1))
                      FROM dataset e
                      JOIN subscription s
                        ON s.id = g.id
                    ...
                    SELECT 'action', e.id, e.data ->> 'name', e.data ->> 'state', e.data ->> 'action_type_name', 'subscription', g.id, 'PARENT', LEAST(2,(g.dart_depth+1))
                      FROM action e
                     WHERE g.type = 'subscription'
                       AND e.data ->> 'state' = 'TEMPLATE'
                    ...
                    SELECT 'trigger', e.id, e.data ->> 'name', e.data ->> 'state', e.data ->> 'trigger_type_name', 'subscription', g.id, 'PARENT', LEAST(2,(g.dart_depth+1))
                      FROM trigger e
                     WHERE g.type = 'subscription'
                       AND e.data #>> '{args,subscription_id}' = g.id
                    ...
                    -- ===============================================
                    SELECT 'workflow', e.id, e.data ->> 'name', e.data ->> 'state', NULL, 'datastore', g.id, 'PARENT', LEAST(2,(g.dart_depth+1))
                      FROM workflow e
                      JOIN datastore d
                        ON d.id = g.id
                    ...
                    -- ===============================================
                    SELECT 'trigger', e.id, e.data ->> 'name', e.data ->> 'state', e.data ->> 'trigger_type_name', 'event', g.id, 'PARENT', LEAST(2,(g.dart_depth+1))
                      FROM trigger e
                     WHERE g.type = 'event'
